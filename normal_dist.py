...
def PDF_beta(x, alpha, beta):
    ...
    val =  beta_dist.ppf(x, alpha, beta)
...
class AdjustMu(Scene):
    ...
    def construct(self):
        ...
        curve = always_redraw(
            lambda: ax.plot(
                lambda x: PDF_beta(x, mu.get_value(), 1), color=YELLOW)
        )
        ...
        self.play(
            mu.animate.set_value(1), run_time=1,
            rate_func=rate_functions.smooth
        )
        self.wait()
        self.play(
            mu.animate.set_value(10), run_time=1.5,
            rate_func=rate_functions.smooth
        )
        self.wait()
        self.play(
            mu.animate.set_value(5), run_time=1,
            rate_func=rate_functions.smooth
        )
